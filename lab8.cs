...
int getAge() const {
    ...
    std::string getAddress() const {  // Новый геттер для адреса
        return address;
    }
...
// Метод для вывода информации о человеке (обновлен для отображения адреса)
...
int main()
{
    ...
    person.setAddress("123 Main St, New York");  // Устанавливаем адрес
    ...
    // Дополнительная проверка работы с адресом
    std::cout << "\nChanging address..." << std::endl;
    person.setAddress("456 Oak Ave, Boston");
    std::cout << "New address: " << person.getAddress() << std::endl;
    person.displayInfo();
